...
const BalancePercentageButtons: React.FC<BalancePercentageButtonsProps> = ({
  ...
  return (
    <div className="flex w-full gap-3 mt-6 mb-2">
      {PERCENTAGES.map((percent) => (
        <button
          ...
          disabled={disabled}
          className={`flex-1 px-3 py-2 rounded-3xl transition-colors duration-200 border-[#1F2530] border-2 text-sm cursor-pointer
